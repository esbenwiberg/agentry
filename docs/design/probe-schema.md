...
> document is the design target. Companion to `primer.md` (architecture
> overview).
...
```ts
import { defineProbe } from '@primer/sdk';
...
```ts
import { fileExists } from '@primer/sdk/recipes';
...
3. **Project config override** — `primer.config.json` has final say.
...
```jsonc
// In primer.config.json (project-side override)
{
  "dimensionOverrides": {
    "agent-context": {
      "probes": {
