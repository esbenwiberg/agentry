...
| # | Doc | What it locks |
|---|---|---|
| 1 | [`trim.md`](./trim.md) | Architecture overview. Three-layer model (engine / probes / evidence). Verbs. Tiers. Reasoned-tier framing. Remediation slot reservation. |
...
| 5 | [`config-and-baseline.md`](./config-and-baseline.md) | `trim.config.json` (the policy gate) and `trim-baseline.json` (the ratchet snapshot). Corpus pinning, gate modes, dimension/probe overrides, waivers, bootstrap flow, `claude-code` LLM provider. |
| 6 | [`reports.md`](./reports.md) | Output formats: human (default) / `--json` / `--ci` / `trim explain`. Visual conventions. Gating-cap rendering. GitHub Actions annotations. |
...
trim is framed as a **successor architecture** to agentry's
scan/brief/add/upgrade/remove/coach/list model. Where agentry manages
artifacts (overlays, practices, briefs), trim measures fitness against
a probe corpus and gates on score.

Until an ADR in `../adr/` formally pivots the project, agentry remains
the shipping codebase and trim is design-only. Whether/when to write
